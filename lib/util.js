...
exports.getVersionByte = function(addr) {
    try {
        ...
        return decoded.slice(0, 1); // version byte
    } catch (e) {
        // Try bech32 or bech32m
        try {
            const hrp = 'atcoin';
            let decoded;

            try {
                // Try Bech32m (witness v1+)
                decoded = bech32m.decode(addr);
            } catch {
                // Try Bech32 (witness v0)
            ...
            const witnessVersion = words[0]; // version byte (0 = P2WPKH, 1 = Taproot)
            return Buffer.from([witnessVersion]);
        } catch (e2) {
...
exports.addressToScript = function(addr) {
    try {
        ...
        if (decoded.length !== 25) {
            console.error('Invalid base58 address length for:', addr);
            throw new Error();
        }

        const version = decoded[0];
        const hash = decoded.slice(1, -4); // pubKeyHash or scriptHash
        ...
        if (version === 0x00) {
            ...
            console.error('Unknown base58 address version byte:', version);
            throw new Error();
        }
    } catch (e) {
        try {
            ...
            try {
                // Try Bech32m (Taproot / witness v1+)
                decoded = bech32m.decode(addr);
            } catch {
                // Try Bech32 (v0: P2WPKH / P2WSH)
                decoded = bech32.decode(addr);
            }

            if (decoded.prefix !== hrp) {
                console.error('Invalid HRP in bech32 address:', addr);
                throw new Error();
            }
            ...
            if (program.length < 2 || program.length > 40) {
                console.error('Invalid witness program length:', program.length);
                throw new Error();
            }

            const versionByte = witnessVersion === 0 ? 0x00 : (0x50 + witnessVersion); // BIP141/BIP350
            return Buffer.concat([
                Buffer.from([versionByte, program.length]),
                Buffer.from(program)
            ]);
        } catch (e2) {
            console.error('Failed to decode address:', addr);
            throw e2;
        }
    }
};
